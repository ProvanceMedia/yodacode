...
- **Consent happens on your laptop, exchange on the server.** The wizard prints a
  ...
  and paste the redirect back. The refresh token is written only to `.env`, so the
...
- **One sign-in per provider.** A provider entry holds one refresh token covering all
  ...
  metadata, meaning the account, the services and the access tiers, is recorded in
...
- `flow: "auth-code"` (the default) needs `authUrl`. The wizard prints a consent link
...
1. **Rotating refresh tokens are handled, as long as the old one survives rotation.**
   ...
   replacement in its private state directory (`broker-state/`, a broker-only volume in
...
2. **The sign-in has to work from a headless server.** The wizard supports the
...
PRs welcome. Copy the shape of the `google` entry.
