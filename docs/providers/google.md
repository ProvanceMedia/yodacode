...
keeps it clean: your mail flows only between your server and Google, with no third
...
- The client ID, the secret and one **refresh token**, stored in `.env`. The broker's
...
3. **Configure the consent screen** at <https://console.cloud.google.com/auth/branding>.
...
4. **Publish the app.** This is the step people miss: <https://console.cloud.google.com/auth/audience>
   ...
   > Google's verification review. An unverified but published app is fine for personal
...
2. Google warns *"Google hasn't verified this app"*. That's expected, because it's **your** app,
...
3. After approving, the browser tries to load `http://127.0.0.1:8765/...` and fails
   with *"This site can't be reached"*, which is **also expected** (that address only means
...
- One Google sign-in covers many services with **one** token. Google's desktop-app flow
  has no "add a scope later", so adding a service means redoing the sign-in. It takes 2 minutes.
...
Google kills refresh tokens when: the consent screen is left in **Testing** (7 days, see
...
- The consent link the wizard prints contains a one-time state nonce and a PKCE
  challenge bound to that terminal session, so a pasted redirect from anywhere else is
  rejected.
