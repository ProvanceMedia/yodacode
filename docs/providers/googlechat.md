...
A two-way Google Chat bot. Easiest is the wizard, which does everything below for you:
...
<https://console.cloud.google.com/?cloudshell=true>, then run this, swapping in your `PROJECT` id:
...
events into your topic, and granting it Publisher is what makes events flow to you.)
...
- **⚠ UNTICK "Build this Chat app as a Workspace add-on" at the very top. Do this FIRST.**
  ...
  `code 13 INTERNAL` and shows "not responding"). It is the single most common failure here,
...
- **Connection settings → Cloud Pub/Sub**, topic `projects/PROJECT/topics/yodacode-chat`
  (it must be in the **same project** as the Chat app, since Google rejects a cross-project topic).
...
> (if you've run `yodacode connect google`). A bot can't *upload* attachments of its own.
> That's a Google restriction, since media upload is user-auth only, so to hand you a file it
...
## If it doesn't reply
Run `yodacode logs`. The pull loop prints any auth or subscription error. Usual causes: the
