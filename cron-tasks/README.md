...
- `model` — every cron yaml must name its model or tier explicitly. No implicit defaults, so you can `head -8 <task>.yaml` and immediately see what it runs on. Prefer a tier — `fast`, `balanced`, `deep`, `extraDeep` — which works whichever AI the assistant runs on. Exact names (`claude-haiku-4-5`, `claude-sonnet-5`, `claude-opus-5`, `claude-fable-5-1`) pin the task to one engine.
