      ...
      try {
        ...
        if (body.command === '/yodacode' || body.command === '/help') {
          ...
          const help = [
            ...
            'everything; then you SSH to the server and run:',
            '```',
            'yodacode addkey',
            '```',
            'It shows what I prepared, you paste the key at a hidden prompt, and (when the service',
            'has a known test endpoint) verifies the connection. Secrets are typed on the *server*,',
            'never into Slack, and live in a separate broker container — I never see them.',
            '',
