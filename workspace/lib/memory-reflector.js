...
// reusable PROCEDURES, memory captures durable FACTS (about Stu, the team,
...
function buildPrompt({ surface, conversationId, userText, replyText, tracker, durationMs }) {
  ...
  return `You are the memory librarian for Yoda, Stu's personal AI agent.
...
- **user-fact**: A new fact about Stu, the team (Tex / Jasper / Scott / Stefan / Josy), or a customer/contact
- **feedback**: Stu corrected your approach, OR confirmed a non-obvious approach worked. Include the WHY.
...
- Trivial timestamps ("Stu said hi at 11am")
...
2. **Append to MEMORY.md** under the right section per the rules already in CLAUDE.md/AGENTS.md:
   ...
   - user-fact / reference / project-state → \`## About Yoda\` (or a new \`##\` section if it really doesn't fit)
