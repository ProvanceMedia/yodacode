...
echo -e "    2. Choose ${B}\"From a manifest\"${X} → pick your workspace → ${B}YAML${X} tab"
echo -e "    3. Paste the contents of this file (shown below) and click ${B}Create${X}:"
echo ""
echo -e "  ${D}──── scripts/slack-app-manifest.yaml ────${X}"
sed 's/^/  /' scripts/slack-app-manifest.yaml
echo -e "  ${D}──────────────────────────────────────────${X}"
echo ""
